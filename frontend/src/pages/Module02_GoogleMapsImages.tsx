...
export default function Module02_GoogleMapsImages({ selectedLocation }: { selectedLocation?: string }) {
    ...
    async function fetchPhotos() {
      ...
      try {
        ...
        if (data.photos && data.photos.length > 0) {
          const foodPhotos = data.photos.slice(0, 3).map((url: string, idx: number) => ({
            url,
            title: idx === 0 ? 'Interior Panoramic' : idx === 1 ? 'Signature Dish' : 'Bar & Băuturi'
          }));
  ...
  const restaurantData = {
    ...
    address: parts.slice(1).join(',').trim() || selectedLocation || '',
    rating: 4.5
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {loading ? (
          ...
          <div className="col-span-3 text-center py-8 text-slate-500">
            <p>Nu s-au găsit poze pentru acest restaurant</p>
          </div>
        )}
      </div>
